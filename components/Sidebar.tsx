...
export function Sidebar({ userRole, isCollapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  const normalizedRole = (userRole === "PROFESSOR" ? "P" : 
                          userRole === "STUDENT" ? "S" : 
                          userRole === "ADMIN" ? "A" : 
                          userRole) as "P" | "S" | "A";

  const filteredItems = navItems.filter(item => item.roles.includes(normalizedRole));
      ...
      <div className={cn(
        ...
        {!collapsed && onToggleCollapse && (
          <button 
            ...
            className="hidden lg:flex p-2 hover:bg-surface-container-highest rounded-xl text-on-surface-variant/30 hover:text-foreground transition-all duration-300"
            title="Recolher Menu"
          >
        ...
        {collapsed && onToggleCollapse && (
          <button 
            onClick={onToggleCollapse}
            className="hidden lg:flex absolute -right-7 top-1/2 -translate-y-1/2 p-1.5 bg-card border border-surface-container-highest text-on-surface-variant/40 hover:text-primary hover:border-primary/30 rounded-full shadow-xl hover:scale-110 transition-all z-50 animate-in fade-in zoom-in duration-500"
            title="Expandir Menu"
          >
