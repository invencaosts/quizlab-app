...
function CommandInput({
  ...
  return (
    <div data-slot="command-input-wrapper" className="p-1 pb-0">
      <InputGroup className={cn("h-12 bg-input/10 border-none rounded-2xl", className)}>
        <CommandPrimitive.Input
          data-slot="command-input"
          className={cn(
            "w-full text-base outline-hidden disabled:cursor-not-allowed disabled:opacity-50 px-4",
            className
          )}
          {...props}
        />
        <InputGroupAddon className="pr-4">
          <SearchIcon className="size-5 shrink-0 opacity-50 text-primary" />
        </InputGroupAddon>
      </InputGroup>
    </div>
  )
